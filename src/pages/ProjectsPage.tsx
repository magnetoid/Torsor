    ...
    <div className="flex bg-page min-h-screen">
      ...
      <main className="flex-1 min-w-0 overflow-y-auto h-screen">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full sm:w-64 bg-surface border border-default rounded-xl pl-9 pr-3 py-1.5 text-sm text-primary outline-none focus:border-accent transition-colors"
                />
              </div>
              <div className="flex items-center bg-surface border border-default rounded-xl p-0.5">
                <button onClick={() => setViewMode('grid')} aria-label="Grid view" aria-pressed={viewMode === 'grid'} className={cn('p-1.5 rounded-lg transition-colors focus-ring', viewMode === 'grid' ? 'bg-elevated text-primary shadow-sm' : 'text-secondary hover:text-primary')}><Grid size={16} /></button>
                <button onClick={() => setViewMode('list')} aria-label="List view" aria-pressed={viewMode === 'list'} className={cn('p-1.5 rounded-lg transition-colors focus-ring', viewMode === 'list' ? 'bg-elevated text-primary shadow-sm' : 'text-secondary hover:text-primary')}><List size={16} /></button>
              </div>
                ...
                <Tooltip.Root>
                  <Tooltip.Trigger asChild>
                    <button
                      ...
                      disabled={creating}
                      className={cn(
                        'flex items-center justify-center gap-2 bg-accent hover:bg-accent-hover text-white px-4 py-1.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-accent/20 relative disabled:opacity-60 focus-ring',
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-default mb-6">
            <div className="flex gap-4 sm:gap-6 overflow-x-auto custom-scrollbar">
              {([['all', 'All'], ['starred', 'Starred'], ['archived', 'Archived']] as const).map(([tab, label]) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={cn('pb-3 whitespace-nowrap text-sm font-medium transition-colors relative focus-ring', activeTab === tab ? 'text-primary' : 'text-secondary hover:text-primary')}
                >
                  {label}
                  <span
                    className={cn(
            ...
            <DropdownMenu.Root>
              <DropdownMenu.Trigger asChild>
                <button className="flex items-center gap-2 text-xs text-secondary hover:text-primary mb-3 sm:mb-0 transition-colors focus-ring rounded-md">
                  <Filter size={14} />
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {isLoading ? (
                  ...
                  <div key={project.id} className="group bg-surface border border-default rounded-xl overflow-hidden hover:border-accent/30 transition-all flex flex-col">
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <DropdownMenu.Root>
                          <DropdownMenu.Trigger asChild>
                            <button aria-label={`Project actions for ${project.name}`} className="p-1 text-secondary hover:text-primary hover:bg-elevated rounded-md transition-colors focus-ring"><MoreVertical size={16} /></button>
                          </DropdownMenu.Trigger>
                          <DropdownMenu.Portal>
                ...
                <div className="md:col-span-2 xl:col-span-3 py-20">
            ...
            <div className="bg-surface border border-default rounded-xl overflow-hidden overflow-x-auto custom-scrollbar">
              {isLoading ? (
                  ...
                  <tbody className="divide-y divide-default">
                    {filteredProjects.map((project) => (
                      <tr key={project.id} className="group hover:bg-elevated transition-colors">
                        ...
                        <td className="px-4 py-3"><button aria-label={`Project actions for ${project.name}`} className="p-1 text-secondary hover:text-primary hover:bg-default rounded-md transition-colors focus-ring"><MoreVertical size={16} /></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
