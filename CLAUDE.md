...
- **`apps/torsor-collab/`** — a small Node **Yjs co-editing sidecar** (`y-websocket`, Phase 7). Opt-in, kept out of the default stack (`docker-compose.collab.yml`). It runs only CRDT sync + awareness; the control-plane proxies `/api/v1/projects/{id}/collab/ws` to it and adds ownership enforcement, so the sidecar is `expose`-only and never reachable directly. Not in the root npm workspaces (its own `package.json`).

`apps/api` and `apps/control-plane` share the **same Postgres schema and `schema_migrations` table** with idempotent SQL, so either service can run migrations against a shared DB without conflict. The migration files were originally **duplicated** in two dirs — [apps/api/migrations/](apps/api/migrations/) and [apps/control-plane/internal/migrations/](apps/control-plane/internal/migrations/). They have since **diverged**: `apps/api` is frozen at `0001`–`0010`, while the control plane (now the backend) has moved ahead through `0011_teams` … `0021_platform_updates_feedback`, all of which exist **only** under `apps/control-plane`. Because new work targets the control plane, add new migrations there; only back-port to `apps/api` if you deliberately need the legacy service to run against the same DB. Whichever dir you touch, keep the SQL idempotent and the numbering monotonic.
...
- **State: Zustand, ~31 stores in [src/stores/](src/stores/)** (`authStore`, `projectStore`, `chatStore`, `editorStore`, etc.), one slice per domain. There is also a separate `src/useAppStore.ts` — a core store used by ~14 IDE files (editor tabs, preview, file tree); it is **live, not dead code** despite the duplicate-looking name.
...
Postgres, UUID PKs, jsonb. Core tables (migrations `0001`+): `users`, `projects` (now carries a nullable `team_id` — see terminology note below), `project_files` (versioned via upsert that bumps `version`), `ai_tasks` (the async queue), `sessions`, `secrets`, `audit_logs`. Later migrations add `workspaces` (0003 — per-project runtime, see note), `usage_events` (0004), `checkpoints` (0006), `deployments` (0007), `task_events` (0008), `mcp_servers` (0009), `workspace_snapshots` (0010), `teams` / `team_members` / `team_invites` (0011), `notifications` (0012), `deployment_events` (0013 — append-only deploy history, distinct from the single-row `deployments` table), `template` column on `projects` (0014 — starter-template provisioning), `memories` (0015 — durable per-project notes/facts/decisions the coding agent and user share via remember/recall tools + the Memory tab), `skills` (0016 — user-defined agent instructions injected into the system prompt when enabled; the Agent Skills tab + `loadEnabledSkills`), `agent_missions` (0017 — decompose a goal into ordered sub-tasks the engine runs autonomously: plan → approve → sequential execution with verify/retry → report; plus a single-row engine config + per-user agent prefs), `learning_proposals` (0018 — the agent's reflection pass stages candidate memories/skills here; nothing touches the real tables until the user approves in the Learning tab), `custom_domains` (0019 — map a user-owned, globally-UNIQUE domain to a project's deployment), `platform_settings` (0020 — a single super-admin config row: maintenance banner + maintenance-mode flag), and `platform_updates` / `feedback` (0021 — the "What's New" changelog + in-app feedback channel, both super-admin-triaged). Note migrations `0011`+ are **control-plane-only** (see the migration note above). The worker reserves pending tasks with `FOR UPDATE SKIP LOCKED` so multiple workers don't double-claim.
